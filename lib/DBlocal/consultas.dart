...
class Controllerconsulta {
  ...
  upKmVehiculoSel(String kilometraje) async {
    var _db = await conection.db;
    try {
      Controllerconsulta().queryVehiculoSel().then((value) async {
      Map<String, dynamic> valoresActualizados = {
        'kilometraje': kilometraje
      };
      var res = await _db.update('AutoSeleccionado', valoresActualizados, where: 'crmid = ?', whereArgs: [value!.crmid],);
      });
    } catch (error) {
      print('En catch '+error.toString());
    }
  }

  //llenar la tabla de viajes para verlos sin conexion
  Future<int> addDataTravel(Viaje viaje) async {
    var _db = await conection.db;
    int result =0;
    try {
      result = await _db.insert('Viaje', viaje.toJSON());
    } catch (e) {
      print(e.toString());
    }
    return result;
  ...
  Future queryDataTravel() async {
    ...
    String fechaAyer = DateTime.now().add(Duration(days: -1)).toString().substring(0, 10);
    var _db = await conection.db;
    List viajeList = [];
    try {
      String consulta = 'SELECT * FROM Viaje WHERE fecha IN ("$fechaHoy", "$fechaAyer") ORDER BY CAST(viajesid AS INTEGER) DESC'; //WHERE fecha = "$fechaHoy" OR fecha = "$fechaAyer" ORDER BY viajesid DESC';
  ...
  Future<int> updateTravelIDs(viajesid, viaje_id, localid) async {
    print('actualizacion de localids '+viajesid+' '+viaje_id+' '+localid);
    var _db = await conection.db;
    int result = 0;
    try {
      Map<String, dynamic> valoresActualizados = {
      ...
      result = await _db.update('ViajesDetalles', valoresActualizados, where: 'viajesid = ?', whereArgs: [localid],);
      int result2 = await _db.update('Viaje', valoresActualizados, where: 'viajesid = ?', whereArgs: [localid],);
    } catch (e) {
      print(e.toString());
    }
    return result;
...
///////////////////////////////////GASTOS///////////////////////////////////////
  ...
  Future<int> addDataGastos(Gastos gastos) async {
    print('insercion de gatos '+gastos.toString());
    var _db = await conection.db;
    int result =0;
    try {
  ...
  Future queryDataGastos() async {
    ...
    List gastosList = [];
    try {
      List<Map<String, dynamic>> maps = await _db.query('Gastos', limit: 100, orderBy: 'gastosid DESC');
      for (var item in maps) {
        gastosList.add(item);
      }
    } catch (e) {
  ...
  Future<int> queryDataGastosID() async {
    ...
    int gastosList = 0;
    // try {
    gastosList = Sqflite.firstIntValue(await _db.rawQuery("SELECT gastosid FROM Gastos order by gastosid DESC")) ?? 0;
    //return travelList.isNotEmpty ? travelList.map((e) => Viaje.fromJSON(e)).toList() : null;
  ...
  Future<int> deleteAllDataGastos() async {
    var _db = await conection.db;
    int result =0;
    try {
      result = await _db.delete('Gastos',);
  ...
  Future<int> addDataGastosDetails(GastoD gastosd) async {
    print(' insecion de detalles del gasto '+gastosd.toString());
    var _db = await conection.db;
    int result = 0;
    try {
